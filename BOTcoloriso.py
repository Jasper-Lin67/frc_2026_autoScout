...
def main(videoIn,videoOut):
    ...
    while True:
        ...
        _, thresh = cv2.threshold(mask_blue, 127, 255, cv2.THRESH_BINARY)
        contours_blue, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        display = frame.copy()
        for cnt in contours_blue:
            if cv2.contourArea(cnt) < 50 or cv2.contourArea(cnt) > 500:
                continue
            x, y, w, h = cv2.boundingRect(cnt)
            if w > 3 and h > 3:
                cv2.rectangle(display, (x, y), (x + w, y + h), (255, 0, 0), 5)

        _, thresh = cv2.threshold(mask_red, 127, 255, cv2.THRESH_BINARY)
        contours_red, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        for cnt in contours_red:
            ...
            x, y, w, h = cv2.boundingRect(cnt)
            if w > 3 and h > 3:
                cv2.rectangle(display, (x, y), (x + w, y + h), (0, 0, 255), 5)
        ...
        cv2.imshow('Blue Mask', cv2.resize(mask_blue, None, fx=1, fy=1, interpolation=cv2.INTER_LINEAR))
