...
def select_roi(title: str, frame: np.ndarray):
    """Open selectROI window; return (x,y,w,h) or None on cancel."""
...
def match_tracks_to_dets(det_centers, tracks):
    ...
    det_centers : list of (cx, cy, w, h) — one per active slot in order
    tracks      : BoT-SORT output array, shape (N, 7)
    Returns     : dict { active_det_index -> track_row }
    """
    matched   = {}
    used      = set()
    for i, (dcx, dcy, dw, dh) in enumerate(det_centers):
        threshold = max(dw, dh) ** 2   # must be within one box-size
...
def main(video_path: str, video_out: str):
    ...
    pending_action = None   # None | 'delete'
    print("\nTracking started.")
    ...
    while cap.isOpened():
        ...
        # ── Delete-mode: waiting for a slot number ───────────────────────────
